...
% Modified by: Andrew LaPre to use as a batch function.
% University of Massachusetts Amherst, Dept. of Kinesiology.
% Copyright 2012.
% Date: Nov.2012.
% Last modified 3/2017 by Andrew LaPre
% -------------------------------------------------------------------------
%
% example function call:
% options.newName{1} = 'Passive_0003'; % build a cell array with new names
% options.TSVfolder = [pwd '\TSV\Passive\GRF_Trimmed\'];
...
function GRF_tsv2mot(options)

nTrials = size(options.newName,2);

for trial = 1:nTrials
%     %%
%     trial = 2
    % Select the force plates for Left leg and Right leg (Total 5 force plates)
    Left_plate = options.leftPlate; % plate 2 belongs to left leg

    % MOT file output filename:
    mot_filename = options.newName{trial};
    write_folder = options.writeFolder;
    ...
    try
        f_cut = options.force_cut; % cut off force at 5N
    catch
        f_cut = 5;
    end
    ...
    try 
        plate_cut_t = options.plate_cut_t;
    catch
        plate_cut_t = [0, 4; 0, 4; 0, 4; 4, 4; 4, 4];
        if ~strcmp(options.autoCut,'Yes')
            warning('specify a 5X2 array for options.plate_cut_t or set options.autoCut = "Yes"');
            options.autoCut = 'No';
        end
    end

    wDir = options.TSVfolder;
...
%         file_input = uigetfile( ...
%         {'*.tsv' 'Qualysis GRF Files (*.tsv)'},'Select files','MultiSelect', 'on', wDir);

    F1Data = dir(fullfile(wDir, '*1.tsv'));
    F2Data = dir(fullfile(wDir, '*2.tsv'));
    F3Data = dir(fullfile(wDir, '*3.tsv'));
    F4Data = dir(fullfile(wDir, '*4.tsv'));
    F5Data = dir(fullfile(wDir, '*5.tsv'));
    file_input{1} = F1Data(trial).name;
    file_input{2} = F2Data(trial).name;
    file_input{3} = F3Data(trial).name;
    file_input{4} = F4Data(trial).name;
    file_input{5} = F5Data(trial).name;
    
    
    
    %% Extract the XYZ forces, moments, & COP (ignore the first 24 columns)
    clear temp grfdata_raw fid c plate_corner
    % grfdata_raw = dlmread(filename,'\t',24,0); %Contain only Fx,Fy,Fz,Mx,My,Mz,COPx,COPy,COPz from 5 force plates, total 9 columns.
    for i = 1:size(file_input,2),
        temp = dlmread([pathname, file_input{i}],'\t',24,0); %Contain only Fx,Fy,Fz,Mx,My,Mz,COPx,COPy,COPz from 5 force plates, total 9 columns.
    ...
    try 
        freq = options.freq; %Hz, from Qualisys
    catch
        freq = 2400;
    end
    ...
    for i = 1:length(file_input),
        fig = figure(i*trial);
        set(gcf,'Position',[50,50, 480,850]);set(fig,'color','w');
        subplot(3,1,1)
    ...
    clear tempFx tempFy tempFz grf tempMx tempMy tempMz tempPx tempPy tempPz copx copy copz COP
    for i = 1:num_plate,
    ...
    for i = 1:num_plate,
    %     if (max(grf(1:200,6*(i-1)+3)) <= 5) % if the max of first 100 Fz ptns is less than threshold 5N
        if (max(abs(grf(1:25,6*(i-1)+2))) <= 5) % if the max of first 100 Fz ptns is less than threshold 5N
    ...
    %     elseif max(grf(end-25:end,6*(i-1)+3) <= 5) % if the max of first 100 Fz ptns is less than threshold 5N
        elseif max(abs(grf(end-25:end,6*(i-1)+2) <= 5)) % if the max of first 100 Fz ptns is less than threshold 5N
    ...
    if strcmp(options.autoCut,'Yes')
        % Plate 1
        for i = 1:size(t_old)
            if abs(grf_clear(i,2)) > f_cut
                plate_cut_t(1,1) = t_old(i-10); 
                break
            end
        end
        % Plate 2
        flag = 0;
        for i = 1:size(t_old)
            if abs(grf_clear(i,8)) > f_cut && flag == 0
                if i > 10
                    plate_cut_t(2,1) = t_old(i-10);
                else
                    plate_cut_t(2,1) = t_old(1);
                end
                flag = 1;
            end
            if abs(grf_clear(i,8)) < f_cut && flag == 1
                if i > size(t_old) - 10
                    plate_cut_t(2,2) = t_old(end);
                else
                    plate_cut_t(2,2) = t_old(i+10);
                end
                break
            end
        end
        ...
        % Plate 3 
        for i = 1:size(t_old)
            if abs(grf_clear(i,14)) < f_cut
                plate_cut_t(3,2) = t_old(i+10);
                break
            end
        end
    end
    ...
    clear grf_new tempx tempy tempz mom_new tempTx tempTy tempTz
    ...
    if Combine_file == 1,
        mot_filename = [mot_filename '_3plates.mot'];
    ...
    dlmwrite(mot_filename, DATA,'-append','delimiter','\t','precision','%.6f');

    % move file to write folder
    movefile(mot_filename,write_folder,'f')
