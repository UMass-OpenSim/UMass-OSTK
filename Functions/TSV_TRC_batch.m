...
% Batch_TSV_TRC(data_folder, write_folder)

function Batch_TSV_TRC(data_folder, write_folder)
...
for trial = 1:nTrials;
    ...
    trc_filename = [write_folder name '.trc'];
    
    foldname = {[write_folder name '_figures']}; % Folder to save figures, change accordingly.
...
dir_c = exist(char(strcat(foldname)), 'dir'); 
if dir_c == 0,
    mkdir(char(strcat(foldname)));
end
...
for j = 1:length(labels), % loop through each markers.
    ...
    if (length(ind) ~= length(data_new(:,k)) && isempty(ind) == 0  ); 
        ...
        legend('Original','Filtered',4);
        print('-djpeg','-r200',char(strcat(foldname,'\Filtered_by_chunks_',num2str(j))));
    % Filter the data for:
    ...
    elseif (length(ind) == length(data_new(:,k))) %Have complete data points
        ...
        ylabel('z-pos','fontweight','bold');
        legend('Original','Filtered',4);
        print('-djpeg','-r200',char(strcat(foldname,'\Filtered_full_',num2str(j))));
        ...
        ylabel('z-pos','fontweight','bold');
        print('-djpeg','-r200',char(strcat(foldname,'\NoFilter_Data_Empty_',num2str(j))));
...
%                 data_interp(first_interp(i):last_interp(i),k:k+2) =[x_interp,y_interp,z_interp]; %Replace the interpolated values to the gap.
            ...
            ylabel('z-pos','fontweight','bold');
            print('-djpeg','-r200',char(strcat(foldname,'\Data_interpolated_',num2str(j))));
            ...
            ylabel('z-pos','fontweight','bold');
            print('-djpeg','-r200',char(strcat(foldname,'\Data_GapTooBig_ToInterpolate_',num2str(j))));
        end
        ...
        plot(t,data_fil(:,k+2),'o');hold on
        ylabel('z-pos','fontweight','bold');
        print('-djpeg','-r200',char(strcat(foldname,'\Data_Complete_Empty_1Chunk_',num2str(j))));
    end
...
% dlmwrite(trc_filename, DATA,'-append','delimiter','\t','precision','%.3f');
