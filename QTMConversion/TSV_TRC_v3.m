...
% STEP 1:
%--------------------------------------------------------------------------
% Select the Proper .TSV file to be process.
% 2 Ways to do this. (1) uncomment the filename = '....' and write the 
% appropriate tsv file name to be process; or (2) hit the 'run button or
% 'F5' to select the appropriate tsv file to process. Make sure the to 
% change the trv filename in STEP 2 before you hit F5. If you have older
% versions of MATLAB, method (1) will certainly work.

% filename = 'S04_Walking0004.trc' %OR uncomment the UI below
[file_input, pathname] = uigetfile( ...
{'*.tsv', 'TSV File (*.tsv)'},'Select files');  

%--------------------------------------------------------------------------
% STEP 2:
%--------------------------------------------------------------------------
% Output TRC file filename (Change accordingly):
% S07 is Russell.
% trc_filename='20121109_S07_Calibration.trc'
% foldname = {'S07_Trial01_Filter_TSV_TRC_v3_'}; % Folder to save figures, change accordingly.
% trc_filename='20121109_S07_Calibration_locked.trc'
% foldname = {'S07_Calibration_locked_Filter_TSV_TRC_v3_'}; % Folder to save figures, change accordingly.
trc_filename='A01_FAST_T0021.trc'
foldname = {'A01_SLOW_Filter_TSV_TRC'}; % Folder to save figures, change accordingly.
% trc_filename='20121109_S07_Trial03_Data_TSV_TRC_v3.trc'
% foldname = {'S07_Trial05_Filter_TSV_TRC_v3_'}; % Folder to save figures, change accordingly.
% trc_filename='20121109_S07_Trial05_Data_TSV_TRC_v3.trc'
% foldname = {'S07_Trial07_Filter_TSV_TRC_v3_'}; % Folder to save figures, change accordingly.
% trc_filename='20121109_S07_Trial07_Data_TSV_TRC_v3.trc'
% foldname = {'S07_Trial08_Filter_TSV_TRC_v3_'}; % Folder to save figures, change accordingly.
% trc_filename='20121109_S07_Trial08_Data_TSV_TRC_v3.trc'
% foldname = {'S07_BHBK_Trial02_TSV_TRC_v3_'}; % Folder to save figures, change accordingly.
% trc_filename='20121109_S07_BHBK_Trial02_Data_TSV_TRC_v3.trc'
...
% Select which markers to export to TRC file. Uncomment to use.
% Option 1: Select from a list of marker text file.
% Option 2: Export all markers (do the selection in OpenSim.
% -------------------------------------------------------------------------

% % Option 1: This is the markers that will be extract and put in the TRC file:
% labels = textread('Devon_FullMarkerNamelist_walk.txt', '%s', 'delimiter', '\n', ...
%                 'whitespace', '');  %list of all markers available set
% num_marker = length(labels); % # of markers
% 
% for ind = 1:num_marker, %Loop through wanted markers
%     for i = 1:num_marker_old,  %Loop through all markers available in TSV file
%         TF = strcmp(labels(ind),label_header(i)); % Perform string comparison
%         if (TF == 1),
%             tempx = data(:,3*(i-1)+1);
%             tempy = data(:,3*(i-1)+2);
%             tempz = data(:,3*(i-1)+3);
%             data_new(:,(3*(ind-1)+1):(3*(ind-1)+3)) = [tempy,tempz,tempx];
%         end
%     end
% end

% Option 2: Export all markers available in TSV file:
labels = label_header;
